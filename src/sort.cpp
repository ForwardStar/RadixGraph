...
std::vector<int> Optimizer::OptimalFanout(int n, int bit_length, int l) {
    std::vector<std::vector<double>> g(l);
    std::vector<std::vector<int>> pre(l);
    for (int i = 0; i < l; i++) {
        g[i].assign(bit_length + 1, (1ll << 62));
    ...
    for (int i = 1; i < l; i++) {
        for (int j = 0; j <= bit_length; j++) {
            for (int k = 0; k <= j; k++) {
                double cost = (1.0 - pow(double((1ll << bit_length) - (1ll << bit_length - k)) / (1ll << bit_length), n)) * (1ll << j);
...
std::vector<int> Optimizer::Prune(std::vector<int> a) {
    std::vector<int> res;
    for (auto x : a) {
        if (x == 0) continue;
        res.push_back(x);
...
SORT::SORT(int n, int bit_length, int l, bool prune) {
    std::vector<int> s = Optimizer::OptimalFanout(n, bit_length, l);
    if (prune) s = Optimizer::Prune(s);
    depth = s.size();
    num_bits = s;
    sum_bits.resize(depth);
    for (int i = 0; i < depth; i++) {
        sum_bits[i] = (i > 0 ? sum_bits[i - 1] : 0) + num_bits[i];
    }
}
